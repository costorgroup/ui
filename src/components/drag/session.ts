import { dragItemClasses } from './drag-item/classes';
import { dragGroupClasses } from './drag-group/classes';
import { DRAG_GROUP_ATTR, DRAG_ITEM_ATTR } from './context';
import {
  TDragBehaviour,
  TDragLockAxis,
  TDragOrientation,
...
type TDragSession = {
  ...
  width: number;
  height: number;
  currentHost: TDragHost | null;
  currentIndex: number | null;
  entered: Set<string>;
};

const hosts = new Map<string, TDragHost>();
let session: TDragSession | null = null;

export const registerDragHost = (host: TDragHost) => {
...
const itemSelector = `:scope > [${DRAG_ITEM_ATTR}="true"]`;

const getItems = (host: TDragHost) =>
  Array.from(host.node.querySelectorAll<HTMLElement>(itemSelector));

const acceptsDrop = (host: TDragHost, source: TDragHost, payload: unknown) => {
...
const findHostAtPoint = (x: number, y: number, source: TDragHost, payload: unknown) => {
  ...
  for (const element of stack) {
    ...
    const root = element.closest(`[${DRAG_GROUP_ATTR}="true"]`);
...
const visibleItems = (host: TDragHost, sourceItem: HTMLElement) =>
...
const movePlaceholder = (
  ...
  const hostsToAnimate = new Set<TDragHost>([host]);
...
const autoScrollHost = (host: TDragHost, x: number, y: number) => {
  if (!host.autoScroll) {
    return;
  }
...
const setGhostPosition = (
  ...
  offsetX: number,
  offsetY: number,
  lockAxis: TDragLockAxis | undefined,
  startX: number,
  startY: number,
) => {
...
const clearItemShift = (host: TDragHost) => {
...
const endDrag = (cancelled = false) => {
  ...
  current.sourceItem.classList.remove(dragItemClasses.dragging);
  current.source.node.classList.remove(dragGroupClasses.dragging);
  current.currentHost?.node.classList.remove(dragGroupClasses.dragging);
...
const onPointerMove = (event: PointerEvent) => {
  ...
  if (host) {
    ...
    if (session.currentHost && session.currentHost.id !== host.id) {
      ...
      session.currentHost.node.classList.remove(dragGroupClasses.dragging);
    }
    ...
    if (session.currentHost?.id !== host.id || session.currentIndex !== nextIndex) {
      ...
      host.node.classList.add(dragGroupClasses.dragging);
      session.currentHost = host;
      session.currentIndex = nextIndex;
    }
...
export const startDragItemDrag = ({
  event,
  host,
  item,
}: {
  ...
  ghost.classList.add(dragItemClasses.ghost);
  ...
  ghost.removeAttribute(DRAG_ITEM_ATTR);

  placeholder.className = dragGroupClasses.placeholder;
  ...
  item.classList.add(dragItemClasses.dragging);
  host.node.classList.add(dragGroupClasses.dragging);
...
export const getDragHost = (id: string) => hosts.get(id);
